...
const int ALLOC_ITERATIONS_IN_THRESHOLD = 10;
...
static void expectThresholdCalls(int calls) {
	expect_zu_eq(hook_calls, calls, "Hook called the right amount of times");
	expect_true(last_peak < chunk_size * 2, "We allocate chunk_size at a time");
	expect_true(last_alloc >= threshold_bytes * calls + alloc_baseline, "Crosses");
}

static void allocateChunks(int chunks) {
...
TEST_BEGIN(test_prof_threshold_hook) {
	...
	allocateChunks(ALLOC_ITERATIONS_IN_THRESHOLD);
	expectThresholdCalls(1);
